...
#include <set> // Für Tracking bereits ausgegebener Nachrichten

// Globale Datenstruktur zum Speichern bereits ausgegebener Nachrichten
std::set<String> loggedMessages;
...
{		  
  ...
  // Configure DIL-Switches as inputs
  pinMode(DILSW1, INPUT_PULLDOWN); // internal pulldown active
...
// Logging-Funktion
void logMessage(const String &functionName, const String &message, const String &values = "")
{
  String fullMessage = "[" + functionName + "] " + message;
  if (values != "")
  {
    fullMessage += ": " + values;
  }

  // Prüfen, ob Nachricht bereits ausgegeben wurde
  if (loggedMessages.find(fullMessage) == loggedMessages.end())
  {
    Serial.println(fullMessage);
    loggedMessages.insert(fullMessage); // Nachricht speichern
  }
}

void modeselection()
{
  if (digitalRead(DILSW1) == 1)
  {
    MODE = 2; // balancing on corner
    logMessage("modeselection", "Mode selected", "Corner balancing activated");
  }
  else
  {
    MODE = 0;
    logMessage("modeselection", "Mode selected", "X side balancing activated");
  }
}
void loop()
{
...
void controller()
{
  ...
  if (abs(phi) <= phi_lim && !flag_terminate)
  {
    ...
    {
      att_tra.generate();
      logMessage("controller", "generate() method", "activated");
    }
    else
    {
      logMessage("controller", "generate() method", "deactivated");
    }
  ...
  if (MODE == 0)
  {
    // Nur Motor 1 aktivieren
    motor1.set_torque(tau_1);
    logMessage("controller", "Motors activated", "Motor 1 only");
  }

  if (MODE == 1)
  {
    // Nur Motor 2 aktivieren
    motor2.set_torque(tau_2);
    logMessage("controller", "Motors activated", "Motor 2 only");
  }
  ...
  if (MODE == 2)
  {
    ...
    logMessage("controller", "Motors activated", "Motor 1, 2 & 3");
  }
}
